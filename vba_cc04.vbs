...
Sub Export()
	...
	For idx = 1 To m_sheetCount
	   ...
	   'move the target sheet
		...
		m_infoRow = rngInfo.rows.Count
		m_infoCol = rngInfo.Columns.Count

		m_dataBgnRow = rngData.row
		m_dataBgnCol = rngData.Column

		m_dataRow = rngData.rows.Count - 2
		m_dataCol = rngData.Columns.Count
...
Sub PutBin(s As String, t As Integer)
	Dim sTmp As String
...
Sub StringToByteArray(dst() As Byte, src As String)
...
Sub WriteTxt(sheetIdx As Long)
	...
	Print #1, m_sheetName & "_info_schema_n = {" & schema_n & "}"
	Print #1, m_sheetName & "_info_schema_t = {" & schema_t & "}"
