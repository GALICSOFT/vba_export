...
Sub WriteBin(sheetIdx As Long)
	...
	For i = 1 To g_infoCol
		...
		schema_name = g_schema(i).name
		Put #2, , schema_name

		ReDim tmpSpc(31 - Len(schema_name))
		Put #2, , tmpSpc

		schema_type = g_schema(i).type
		Put #2, , schema_type

		ReDim tmpSpc(15 - Len(schema_type))
		Put #2, , tmpSpc
	Next
	...
	'write the record
	For i = 1 To g_infoRow
		For j = 1 To g_infoCol
			sType = g_schema(j).type

			recStr = Trim(Cells(g_infoBgnRow + 1 + i, g_infoBgnCol - 1 + j))

			If "function" = sType Or "string" = sType Then
				strLen = Len(recStr)
				Put #2, , strLen
				Put #2, , recStr

			ElseIf "int" = sType Then
				recInt = CLng(recStr)
				Put #2, , recInt
